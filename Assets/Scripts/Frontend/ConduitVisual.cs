...
public class ConduitVisual : MonoBehaviour, IPointerClickHandler
{
    ...
    public float verticalOffset = 0.582f;
...
// ---- END KNOT ----
    ...
    private void SplineFromPath()
    {
        ...
        {
            BezierKnot knot = new BezierKnot(path[i] + Vector3.up * verticalOffset);
    ...
    private void ColorSplineIfValid()
    {
        ...
        Vector2Int[] trimmedCells = new Vector2Int[newLength];
        Array.Copy(cells, 0, trimmedCells, 0, newLength);
        if (enoughBridgesToFinish &&
            planeA.IsPlaceOccupied(planeA.ToPlaneLocal(planeA.WorldToLocal(path.Last()))) is TimeRipple ripple)
        {
            pipeMaterial.SetColor("_Color", validColor);
            pipeMaterial.SetColor("_Color2", validColor);
        }
        else if (enoughBridgesToFinish &&
                 GameFrontendManager.Instance.IsConnectionPathValid(sourceNodeVisual.layerNum, trimmedCells))
        {
